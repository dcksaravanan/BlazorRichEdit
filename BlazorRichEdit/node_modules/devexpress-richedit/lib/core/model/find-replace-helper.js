...
var FindReplaceHelper = (function () {
    function FindReplaceHelper(modelManager, layoutFormatterManager, selection, subDocument, layout, storeSelection) {
    ...
    FindReplaceHelper.prototype.findNextDownInternal = function (lowerPosition, greaterPosition) {
        var charIterator = new ForwardCharacterIterator(this.modelManager, this.layoutFormatterManager, this.selection, this.subDocument, lowerPosition, greaterPosition);
    ...
    FindReplaceHelper.prototype.findNextDownWholeWordsOnly = function (charIterator, lowerPosition, greaterPosition) {
        ...
        if (this.layout.pages[0].getPosition() < lowerPosition &&
            this.isCharCanBeInWord((this.subDocument.isMain() ? new layout_position_creator_1.LayoutPositionMainSubDocumentCreator(this.layout, this.subDocument, lowerPosition - 1, document_layout_details_level_1.DocumentLayoutDetailsLevel.Character)
                : new layout_position_creator_1.LayoutPositionOtherSubDocumentCreator(this.layout, this.subDocument, lowerPosition - 1, this.selection.pageIndex, document_layout_details_level_1.DocumentLayoutDetailsLevel.Character))
    ...
    FindReplaceHelper.prototype.findNextUpInternal = function (lowerPosition, greaterPosition) {
        ...
        var charIterator = new BackwardCharacterIterator(this.modelManager, this.layoutFormatterManager, this.selection, this.subDocument, lowerPosition, greaterPosition);
        if (this.wholeWordsOnly)
    ...
    FindReplaceHelper.prototype.findNextUpWholeWordsOnly = function (charIterator, greaterPosition) {
        var offset = 0;
        var layoutPage = this.subDocument.isMain() ? this.layout.getLastValidPage() : this.layout.pages[this.selection.pageIndex];
        var posWhenStartEquivalents = -1;
        if (greaterPosition < layoutPage.getEndPosition() - 1 &&
            this.isCharCanBeInWord((this.subDocument.isMain() ? new layout_position_creator_1.LayoutPositionMainSubDocumentCreator(this.layout, this.subDocument, greaterPosition + 1, document_layout_details_level_1.DocumentLayoutDetailsLevel.Character)
                : new layout_position_creator_1.LayoutPositionOtherSubDocumentCreator(this.layout, this.subDocument, greaterPosition + 1, this.selection.pageIndex, document_layout_details_level_1.DocumentLayoutDetailsLevel.Character))
    ...
    FindReplaceHelper.prototype.replaceAll = function (startWithPos) {
        ...
        while (this.findNextAll() == FindReplaceState.Found)
            resultIntervals.push(new linked_interval_1.LinkedInterval(this.subDocument.positionManager, this.lastFound));
        if (resultIntervals.length) {
            ...
            for (var _i = 0, resultIntervals_1 = resultIntervals; _i < resultIntervals_1.length; _i++) {
                ...
                var charStyle = firstRun.characterStyle;
                var maskedCharProp = firstRun.maskedCharacterProperties;
                modelManager.modelManipulator.range.removeInterval(new sub_document_1.SubDocumentInterval(this.subDocument, curr.getFixedInterval().clone()), true, false);
...
var CharacterIteratorBase = (function () {
    function CharacterIteratorBase(modelManager, layoutFormatterManager, selection, subDocument, startPosition, endPosition) {
        this.modelManager = modelManager;
        this.subDocument = subDocument;
        this.iterator = subDocument.isMain() ?
            ...
            new layout_box_iterator_other_sub_document_1.LayoutBoxIteratorOtherSubDocument(subDocument, layoutFormatterManager.layout, startPosition, endPosition, selection.pageIndex);
