...
var AbstractNumberingListDestination = (function (_super) {
    ...
    AbstractNumberingListDestination.prototype.processElementClose = function (_reader) {
        if (string_1.StringUtils.isNullOrEmpty(this.id))
            return;
        if (this.list.innerId == -1) {
            var innerId_1 = 1;
            while (list_1.ListUtils.elementBy(this.data.documentModel.abstractNumberingLists, function (list) { return list.innerId == innerId_1; }))
                innerId_1++;
            this.list.innerId = innerId_1;
        }
        var index = this.data.documentModel.abstractNumberingLists.length;
