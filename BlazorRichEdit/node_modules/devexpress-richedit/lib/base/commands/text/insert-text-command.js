...
var InsertTextCommand = (function (_super) {
    ...
    InsertTextCommand.prototype.correctPrevHistoryRun = function (params) {
        ...
        if (currDate.getTime() - this.control.commandManager.lastTextInsertDate.getTime() < 2.7 * 1000) {
            var insertTextHistoryInfo = this.modelManipulator.text.getLastModifiableHistoryItem(function (hi) { return hi instanceof insert_text_history_item_1.InsertTextHistoryItem; });
            var selectionHistoryItem = this.modelManipulator.text.getLastModifiableHistoryItem(function (hi) { return hi instanceof selection_history_item_1.SelectionHistoryItem; });
            if (insertTextHistoryInfo && selectionHistoryItem) {
                var oldParams = insertTextHistoryInfo.params;
                if (oldParams.subDocPos.subDocument.id == params.subDocPos.subDocument.id &&
                    ...
                    !selectionHistoryItem.newState.intervalsInfo.multiselection &&
                    selectionHistoryItem.newState.intervalsInfo.intervals[0].length == 0 &&
                    selectionHistoryItem.newState.intervalsInfo.interval.start == params.subDocPos.position) {
                    this.modelManipulator.text.insertTextInner(params);
                    selectionHistoryItem.newState.intervalsInfo.interval.start += params.text.length;
                    oldParams.text += params.text;
                    var newPos_1 = selectionHistoryItem.newState.intervalsInfo.interval.start;
