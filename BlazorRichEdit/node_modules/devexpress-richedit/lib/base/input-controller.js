...
var InputEditorBase = (function () {
    ...
    InputEditorBase.prototype.onInput = function (_evt) {
        var _this = this;
        if (this.canInsertTextOnInputEvent)
            this.onInputTimerId = setTimeout(function () { return _this.onTextInput(); }, 0);
    };
    ...
    InputEditorBase.prototype.onKeyPress = function (evt) {
        ...
        if (!evt.altKey) {
            if (this.inputWithAlt)
                this.keyPressTimerId = setTimeout(function () { return _this.onTextInput(); }, 0);
            this.inputWithAlt = false;
        }
    };
...
var IFrameInputEditor = (function (_super) {
    ...
    IFrameInputEditor.prototype.initEvents = function () {
        ...
        this.evtHandlersHolder.addListener(this.getEditableDocument(), "paste", function (evt) {
            ...
            if (evt && evt.clipboardData && evt.clipboardData.items) {
                ...
                var rtfItem = data_transfer_1.DataTransferUtils.getRtfTextItem(items);
                if (rtfItem) {
                    evt_1.EvtUtils.preventEvent(evt);
                    _this.insertClipboardRtf(rtfItem);
                    return;
                }
                if (!data_transfer_1.DataTransferUtils.getPlainTextItem(items)) {
                    var file = data_transfer_1.DataTransferUtils.getImageItem(items);
                    if (file) {
                        evt_1.EvtUtils.preventEvent(evt);
                        _this.insertClipboardPicture(file);
                        return;
                    }
                }
            }
    ...
    IFrameInputEditor.prototype.insertClipboardPicture = function (file) {
        var _this = this;
    ...
    IFrameInputEditor.prototype.insertClipboardRtf = function (rtfItem) {
        var _this = this;
        this.control.isLoadingPictureFromClipboard = true;
        var loadingPanelManager = this.control.loadingPanelManager.loadingPanel.showPanelDelayed(400);
        rtfItem.getAsString(function (rtf) {
            if (rtf.length > 100000)
                loadingPanelManager.executeIfTimerExpired();
