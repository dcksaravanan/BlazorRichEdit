...
var RulerTabsControl = (function (_super) {
    ...
    RulerTabsControl.prototype.onMouseDown = function (source, evt) {
        ...
        if (!this.currModelState.enabled || this.modelData.isReadOnly)
            return false;
        var canBeHandled = list_1.ListUtils.unsafeAnyOf(this.subControls, function (subControl, index) {
            if (subControl.canHandle(source)) {
                _this.tabAction = TabAction.Move;
        ...
        if (!canBeHandled) {
            ...
            if (this.isTabMarkZone(evt) && pos >= this.minPosition() && pos <= this.maxPosition()) {
                ...
                canBeHandled = true;
                this.newTab = new paragraph_style_1.TabInfo(pos - this.controls.paragraphLeftPosition, this.controls.tabTypeBox.align, layout_tab_space_box_1.TabLeaderType.None, false, false);
    ...
    RulerTabsControl.prototype.minPosition = function () {
        return this.controls.paragraphLeftPosition +
            Math.min(this.controls.leftIndent.currModelState.modelValue, this.controls.firstLineIndent.currModelState.modelValue);
    };
    RulerTabsControl.prototype.maxPosition = function () {
        return this.controls.paragraphRightPosition - this.controls.rightIndent.currModelState.modelValue;
    };
    RulerTabsControl.prototype.calculateNewModelState = function (distance) {
        switch (this.tabAction) {
            case TabAction.Insert: {
                ...
                var parLeft = this.controls.paragraphLeftPosition;
                var prevPos = parLeft + startPos;
                currTab.position = this.controls.chooseClosestAnchorPosition(prevPos + distance, [prevPos], new min_max_1.MinMaxNumber(this.minPosition(), this.maxPosition())) - parLeft;
                break;
            }
            case TabAction.Move: {
