...
function insertRtfInSubDocumentPublic(modelManager, subDocument, position, rtf, batchUpdatableObject, callback) {
    var options = new importer_options_2.RtfImporterOptions(function () { });
    new rtf_importer_1.RtfImporter(options).importFromString(rtf, modelManager.richOptions, getAfterInsertCallback(modelManager, subDocument, position, batchUpdatableObject, callback), getAfterInsertReject(callback));
}
exports.insertRtfInSubDocumentPublic = insertRtfInSubDocumentPublic;
function insertContentInSubDocumentPublic(modelManager, subDocument, position, content, batchUpdatableObject, documentFormat, callback) {
    ...
    importer.importFromFile(content, modelManager.richOptions, getAfterInsertCallback(modelManager, subDocument, position, batchUpdatableObject, callback), getAfterInsertReject(callback));
}
exports.insertContentInSubDocumentPublic = insertContentInSubDocumentPublic;
function getAfterInsertCallback(modelManager, subDocument, position, batchUpdatableObject, callback) {
    return function (model, formatImagesImporter) {
        formatImagesImporter.whenAllPicturesLoaded(function (_success) {
            batchUpdatableObject.beginUpdate();
            var interval = modelManager.modelManipulator.subDocument.insertSubDocument(new sub_document_1.SubDocumentPosition(subDocument, position), new sub_document_1.SubDocumentInterval(model.mainSubDocument, model.mainSubDocument.interval));
            batchUpdatableObject.endUpdate();
            callback(interval, true);
        }, 5000);
        var clientModelManager = new model_manager_1.ClientModelManager(model, modelManager.richOptions, new batch_updatable_1.EmptyBatchUpdatableObject());
