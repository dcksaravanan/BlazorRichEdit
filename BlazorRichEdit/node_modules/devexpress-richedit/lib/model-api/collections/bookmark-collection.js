...
var BookmarkCollection = (function (_super) {
    ...
    BookmarkCollection.prototype.find = function (position) {
        ...
        return list_1.ListUtils.map(parameter_checker_1.ApiParametersChecker.check(position, 1, false, [
            parameter_checker_1.ApiParametersChecker.numberDescriptor("position", function (pos) { return bookmark_finder_1.findBookmarkByInterval(sd, [new fixed_1.FixedInterval(pos, 0)]); }, 0, constants_1.Constants.MAX_SAFE_INTEGER),
            model_parameter_checker_1.ModelParametersChecker.intervalDescriptor("interval", function (interval) { return bookmark_finder_1.findBookmarkByInterval(sd, [new fixed_1.FixedInterval(interval.start, interval.length)]); }, 0, constants_1.Constants.MAX_SAFE_INTEGER),
            model_parameter_checker_1.ModelParametersChecker.intervalsDescriptor("intervals", function (apiIntervals) {
                return bookmark_finder_1.findBookmarkByInterval(sd, list_1.ListUtils.map(apiIntervals, function (interval) { return new fixed_1.FixedInterval(interval.start, interval.length); }));
